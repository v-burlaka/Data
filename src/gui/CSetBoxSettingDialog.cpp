...
void CSetBoxSettingDialog::initBloks(std::vector<sBlockCoord>& emptyBlocks)
{
   emptyBlocks.clear();
   ...
   for(auto table : inputDialogs)
   {
      ...
      coord.startCoord.X = table->lineEdit_x1->text().toInt();
      coord.startCoord.Y = table->lineEdit_y1->text().toInt();
      coord.startCoord.Z = table->lineEdit_z1->text().toInt();

      coord.endCoord.X = table->lineEdit_x2->text().toInt();
      coord.endCoord.Y = table->lineEdit_y2->text().toInt();
      coord.endCoord.Z = table->lineEdit_z2->text().toInt();

      emptyBlocks.push_back(coord);
   }
}

void CSetBoxSettingDialog::initBloks(std::vector<sBoxInfo>& bloks)
{
   bloks.clear();
